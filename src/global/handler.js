...
export default {
  ...
  codeHandler(code, msg) {
    switch (code) {
      case 401:
        mq.msgError(msg ?? '你没有权限访问此页面，请先登录')
        setTimeout(() => {
          toLogin()
        }, 1000)
        break
      case 403:
        mq.msgError(msg ?? '登录过期，请重新登录')
        setTimeout(() => {
          toLogin()
        }, 1000)
        break
      case 404:
        mq.msgError(msg ?? '请求失败，请检查网络连接或联系管理员')
        break
      case 500:
        mq.msgError(msg ?? '服务器内部错误，请稍后再试')
        toError(500)
        break
      case 530:
        mq.msgError(msg ?? '账号或密码错误！')
        break
      default:
        mq.msgError(msg ?? '未知错误，请稍后再试')
    }
  },
